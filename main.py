...
def main():
    ...
    # ========== هندلرهای پیام از ادمین (با گروه‌بندی) ==========
    # گروه 1: ذخیره محصول جدید
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & ~filters.Regex(r'^hahbyhh555466mamabbbnn$'), 
        save_new_product
    ), group=1)
    
    # گروه 2: ذخیره ویرایش فیلد محصول
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & ~filters.Regex(r'^hahbyhh555466mamabbbnn$'), 
        save_edit_field
    ), group=2)
    
    # گروه 3: ذخیره ویرایش ظاهر
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & ~filters.Regex(r'^hahbyhh555466mamabbbnn$'), 
        save_ui_setting
    ), group=3)
    
    # گروه 4: ارسال پیام همگانی
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & ~filters.Regex(r'^hahbyhh555466mamabbbnn$'), 
        handle_broadcast_message
    ), group=4)
    
    # گروه 5: ارسال پیام به کاربر (از بخش چت)
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & ~filters.Regex(r'^hahbyhh555466mamabbbnn$'), 
        handle_reply_to_user
    ), group=5)
    
    # گروه 6: ارسال دستی محصول به خریدار (بعد از تأیید)
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & ~filters.Regex(r'^hahbyhh555466mamabbbnn$'), 
        handle_admin_reply
    ), group=6)
    
    # ========== دکمه‌های پنل ادمین (CallbackQuery) ==========
    # محصولات
